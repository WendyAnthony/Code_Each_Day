...
Modified Date & Time: 2025/08/31 @ 22:02
...
- Save each worksheet to 2 separate csv files `data/SiteDetails-TEST-GOE-Monitor.csv` & `data/SiteData-TEST-GOE-Monitor.csv`
...
## Randomly Change Numbers to Create New Data Set for Year 2
```{r random-df-yr2}
geo_eccc_random_2024 <- read.csv("data/TEST-GOE-Monitor-randomChange-diffYear-TEST.csv", header = TRUE, sep = ",")

library(dplyr)
# https://stackoverflow.com/questions/73773734/adding-a-random-value-to-each-value-in-the-data-conditional-on-the-column-means
# this adds some values that are negative
# geo_eccc_random_2024 <- geo_eccc_random_2024 %>%
#  mutate(across(where(is.numeric), \(x) x + rnorm(n(), mean = 0, sd = 0.5 * mean(x))))
# need to avoid one column year
str(geo_eccc_random_2024)
# change year to date for the moment
# geo_eccc_random_2024$Year1 <- geo_eccc_random_2024$Year
# https://stackoverflow.com/questions/69555486/how-can-i-convert-a-column-of-date-time-data-from-numeric-to-character-in-r

geo_eccc_random_2024$Year <- as.character(geo_eccc_random_2024$Year) 
str(geo_eccc_random_2024) 

geo_eccc_random_2024[] <- lapply(geo_eccc_random_2024, function(x) {
  if(!is.numeric(x)) x else x + rnorm(length(x), 0, 0.1 * mean(x))
  })
geo_eccc_random_2024
```

### Data Cleaning and Transformation
<details><summary><b><i>Click open to see R code for `Data Cleaning and Transformation`</i></b></summary><p>
```{r, data-cleaning-random-newYear}

# check each column for negative value
# https://stackoverflow.com/questions/50078917/replace-negative-values-with-positive-values-in-r
# z<--pmin(-z, z) 
geo_eccc_random_2024$Proportion_of_native_species <--pmin(-geo_eccc_random_2024$Proportion_of_native_species, geo_eccc_random_2024$Proportion_of_native_species) 

geo_eccc_random_2024$Cultural_species_richness <--pmin(-geo_eccc_random_2024$Cultural_species_richness, geo_eccc_random_2024$Cultural_species_richness) 

geo_eccc_random_2024$Exotic_species <--pmin(-geo_eccc_random_2024$Exotic_species, geo_eccc_random_2024$Exotic_species) 

geo_eccc_random_2024$Trampling <--pmin(-geo_eccc_random_2024$Trampling, geo_eccc_random_2024$Trampling) 

geo_eccc_random_2024$Herbivory <--pmin(-geo_eccc_random_2024$Herbivory, geo_eccc_random_2024$Herbivory) 

geo_eccc_random_2024$Composite_Index <--pmin(-geo_eccc_random_2024$Composite_Index, geo_eccc_random_2024$Composite_Index) 

names(geo_eccc_random_2024)
str(geo_eccc_random_2024)

# https://www.reddit.com/r/rstats/comments/wfn1rf/forcing_visualizations_in_r_to_have_only_2/
# round numbers to 2 digits
library(dplyr)

geo_eccc_random_2024 %>% mutate_if(is.numeric, round, digits=2) -> geo_eccc_random_2024

str(geo_eccc_random_2024)

# create new column with proportions changed to percentage as a whole number
# As this Makes Anniversary Island Proportion >1 change to 1
# data.frame[row_number, column_number] = new_value
# geo_eccc_random_2024[1, 3] <- 1
# but if more than one row ends up being more than 1
# https://stackoverflow.com/questions/66398854/r-set-column-to-maximum-of-current-entry-and-specified-value-in-an-elegant-way
# this made them all to 1
# geo_eccc_random_2024$Proportion_of_native_species <- pmax(geo_eccc_random_2024$Proportion_of_native_species, 1)
# https://stackoverflow.com/questions/66398854/r-set-column-to-maximum-of-current-entry-and-specified-value-in-an-elegant-way
# data.table version data[foo < 1, foo := 1])
# doesn't work
# geo_eccc_random_2024[geo_eccc_random_2024$Proportion_of_native_species > 1.0000, geo_eccc_random_2024$Proportion_of_native_species == 1.0000]

# change Year back to number
geo_eccc_random_2024$Year <- as.numeric(as.character(geo_eccc_random_2024$Year))
str(geo_eccc_random_2024) 

# WHAT TO DO IF PROPORTION OF NATIVE SPECIES is >1

geo_eccc_random_2024

Percentage_ns <- geo_eccc_random_2024$Proportion_of_native_species * 100

# add-new column for Percentages
geo_eccc_random_2024 <- cbind(geo_eccc_random_2024, Percentage_ns)

# arrange order of site names
Sites_eccc_1 <- Sites_eccc %>% arrange(Sites_eccc$Site)
geo_eccc_random_2024_1 <- geo_eccc_random_2024 %>% arrange(geo_eccc_random_2024$Site)
...
geo_eccc_random_2024_sites <- cbind(Sites_eccc_1, geo_eccc_random_2024_1)

# Remove duplicate columns resulting from binding 2 data frames
geo_eccc_random_2024_sites_1 <- geo_eccc_random_2024_sites[c(-9,-10)]

# Rename columns to remove dot-format
names(geo_eccc_random_2024_sites_1)[names(geo_eccc_random_2024_sites_1) == "Land.Manager"] <- "Land_Manager"
names(geo_eccc_random_2024_sites_1)[names(geo_eccc_random_2024_sites_1) == "Main.Restoration.Type"] <- "Main_Restoration_Type"
names(geo_eccc_random_2024_sites_1)[names(geo_eccc_random_2024_sites_1) == "Restoration.Intensity"] <- "Restoration_Intensity"
names(geo_eccc_random_2024_sites_1)[names(geo_eccc_random_2024_sites_1) == "Area.ha"] <- "Area_ha"

# write csv file with combined data 
## to create new data file which is used for creating the visualizations
write.csv(geo_eccc_random_2024_sites_1, "data/geo_eccc_random_2024_all_site_data.csv", row.names = FALSE)
```
</p></details>
...
# Save map
saveWidget(ECCC_map, "output/ECCC_map_custom_icons.html")
...
# Save map
saveWidget(ECCC_map, "output/ECCC_map_circle_markers.html")
...
# Save map
saveWidget(ECCC_map, "output/ECCC_map_area.html")
...
##  Graph for ER390 Proposal Methods Workflow
```{r ER390-Proposal-mermaid-graph-shapes, echo=FALSE}
DiagrammeR("
graph TB
...
# Images Include when knitting
```{r, echo=FALSE}
    knitr::include_graphics("images/HAT_logo_ss_1.png")
```

# Descriptive Summary Stats
```{r descriptive-summary-stats}
summary(geo_eccc)
```
