...
dat3 <- select(dat,
               SourceFile, DateTimeOriginal, GPSTimeStamp,
               GPSLongitude, GPSLatitude,
...
## map with location ana image, no text
```{r map1-gps}
m <- leaflet(dat2) %>%
  addProviderTiles("Esri.WorldImagery") %>%
  addMarkers(~ GPSLongitude, ~ GPSLatitude,
#             popup = paste0("<img src = '/Volumes/My Passport for Mac/Pictures/!!Nikon-Camera/Nikon-Coolpix-GPS/2019-06-20-LongBeach/", dat2$SourceFile, "' style='width:314px;height:239px;'", ">"))
popup = paste0("<img src = '", dat2$SourceFile, "' style='width:314px;height:239px;'", ">"))
...
htmlwidgets::saveWidget(m, "photo-map-pop1.html")
```

## larger map

```{r map-larger}
# to make map larger (not responsive to changing screen size)
...
saveWidget(m2, "photo-map-pop-2a.html")
```
## popup text
...
saveWidget(m2b, "photo-map-pop-2b.html")
```
...
m3$height <- 1000
#m3
saveWidget(m3, "photo-map-pop-4a.html")
```
...
saveWidget(example.map, "photo-map-pop-3c-ex.html")
getwd()
```

## Responsive map with exif
```{r responsive-map-exif}
...
CameraExif.map <- leaflet(dat3) %>%
  ...
  addMarkers(clusterOption = markerClusterOptions(spiderfyOnMaxZoom = TRUE),
             ~ GPSLongitude, ~ GPSLatitude, icon = greenLeafIconSm,
             popup = paste0("<b>Image Name:</b> ", dat3$SourceFile, "<br><br>", "<b>Image Date:</b> ", dat3$DateTimeOriginal, "<br>", "<b>GPS TimeStamp:</b> ", dat3$GPSTimeStamp, "<br><br>", "<b>F Stop:</b> ", dat3$FNumber, "<br>", "<b>ISO:</b> ", dat3$ISO, "<br>", "<b>Focal Length:</b> ", dat3$FocalLength, "<br>", "<b>Exposure Time:</b> ", dat3$ExposureTime, "<br><br>",
                            "<img src = '",
...
# Error in saveas(example.map, "photo-map-pop-3c-ex.html") :could not find function "saveas"
saveWidget(CameraExif.map, "CameraExifMap-a.html")
getwd()
...
# Error in saveas(example.map, "photo-map-pop-3c-ex.html") :could not find function "saveas"
saveWidget(example.map, "photo-map-pop-4c-ex.html")
getwd()
```
