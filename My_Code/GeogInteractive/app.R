  ...
  setBackgroundColor("bonewhite"),
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        downloadButton("download", "Download CSV", class="butt")
      ),
      mainPanel(
        # leafletOutput("Map"),
...
#   })
  ...
  #https://community.rstudio.com/t/how-do-you-download-the-result-of-the-datatable-search/105355/4
  # output$download <- downloadHandler(
  #   filename = "report.csv",
  #   content = function(file) {
  ...
  data_filtered <- reactive({
    filter(geodata, Course %in% course(), Name %in% name())
    #     filter(pheno_subset_sub, Species %in% species(), CommonName %in% commonName(), LocName %in% locName())
  })

  course <- reactive({
    if (is.null(input$Course)) unique(geodata$Course) else input$Course
  })

  name <- reactive({
    if (is.null(input$Name)) unique(geodata$Name) else input$Name
  })
