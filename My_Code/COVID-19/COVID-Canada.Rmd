...
bar_graph_data_recent_can_dose1 <- all_data_can_pop %>%  
  filter(Date == max(Date), !(Province %in% not_provinces)) %>%  
  select(Province, PercentVaccinatedDose1_percent)
```
...
bar_graph_dose1 <- ggplot(bar_graph_data_recent_can_dose1, 
                aes(x = reorder(Province, PercentVaccinatedDose1_percent), 
                    y = PercentVaccinatedDose1_percent)) +
  geom_col(color = "black", fill="#0072B2", size = 0.5) +
  theme_minimal() +
  theme(axis.text=element_text(size = 8)) +
  labs(title = "Dose 1 % Vaccinated \nMarch  23, 2022") +
   ylab("") +
   xlab("") +
  coord_flip()
```
...
bar_graph_data_early_dose1 <- all_data_can_pop %>%
  ...
  select(Province, PercentVaccinatedDose1_percent, tooltip_text)
```
...
bar_graph_data_recent_can_dose2 <- all_data_can_pop %>%  
  filter(Date == max(Date), !(Province %in% not_provinces)) %>%  
  select(Province, PercentVaccinatedDose2_percent)
```
...
bar_graph_dose2 <- ggplot(bar_graph_data_recent_can_dose2, 
                aes(x = reorder(Province, PercentVaccinatedDose2_percent), 
                    y = PercentVaccinatedDose2_percent)) +
  geom_col(color = "black", fill="#0072B2", size = 0.5) +
  theme_minimal() +
  theme(axis.text=element_text(size = 8)) +
  labs(title = "Dose 2 % \nVaccinated March  23, 2022",
       subtitle = "Data from https://resources-covid19canada.hub.arcgis.com"
       ) +
   ylab("") +
   xlab("") +
...
bar_graph_data_early_dose2 <- all_data_can_pop %>%
  ...
  select(Province, PercentVaccinatedDose2_percent, tooltip_text)
```
...
bar_graph_data_recent_can_Booster <- all_data_can_pop %>%  
  filter(Date == max(Date), !(Province %in% not_provinces)) %>%  
  select(Province, PercentVaccinatedBooster_percent)
```
...
bar_graph_Booster <- ggplot(bar_graph_data_recent_can_Booster, 
                aes(x = reorder(Province, PercentVaccinatedBooster_percent), 
                    y = PercentVaccinatedBooster_percent)) +
  geom_col(color = "black", fill="#0072B2", size = 0.5) +
  theme_minimal() +
  theme(axis.text=element_text(size = 8)) +
  labs(title = "Booster % Vaccinated \nMarch  23, 2022") +
   ylab("") +
   xlab("") +
  coord_flip()
```
...
bar_graph_data_early_Booster <- all_data_can_pop %>%
  ...
  select(Province, PercentVaccinatedBooster_percent, tooltip_text)
```
...
theme_map <- function(base_size=9, base_family="") {
    ...
    theme_bw(base_size=base_size, base_family=base_family) %+replace%
        theme(axis.line=element_blank(),
              ...
              plot.subtitle = element_text(face = "italic", colour = "black", size = 8, hjust = 0.5, vjust = 1),
              plot.caption = element_text(hjust = 0.5, face = "italic", size = 8, vjust = 1)
              )
}
...
p_can_out <- p_can + geom_sf(color = "gray60", 
  ...
  scale_fill_manual(values = map_colors) + 
  guides(fill = FALSE) + 
      labs(title = "Canada COVID \nvaccination Status 2020-2022",
           subtitle = "(hover over map or chart)",
       caption = "Data from https://resources-covid19canada.hub.arcgis.com"
       ) +
  theme_map() + 
  theme(panel.grid.major = element_line(color = "white"),
        legend.key = element_rect(color = "gray40", size = 0.1))
...
```{r link-map-barchart-can}
 prov_map <- ggplot() +
      labs(title = "Canada COVID vaccination Status 2020-2022",
                      subtitle = "(hover over map or chart for interactivity)",
       caption = "Data from https://resources-covid19canada.hub.arcgis.com"
       ) +
...
HTML knit error  
 "png not found in resource path Error: pandoc document conversion failed with error 99"
...
1. change x axis tick marks to 90 for all
2. make x axis (y) full range for multiples
