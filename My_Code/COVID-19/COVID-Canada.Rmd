...
```{r tooltip-column-%-Booster-recent}
...
```{r tooltip-column-%-early-booster}
...
# Functions for filtering non-province and creating df for specific date


---

## Dose1 TEST
Message: Note: Using an external vector in selections is ambiguous.  (instead of select(Province, x, y))
ℹ Use `all_of(x)` instead of `x` to silence this message.  
ℹ See <https://tidyselect.r-lib.org/reference/faq-external-vector.html>
```{r function-filter-non-prov-dose1-TEST}
...
## Dose1 function test
... instead of having a separate function for each dose, specifying column
To specify which columns and date
fill in (date, "col1, "col2)
```{r function-filter-non-prov-dose-test}
dose1_TEST <- dose_TEST(2021-06-17, "PercentVaccinatedDose1", "PercentVaccinatedDose1_percent")
```

---

### Don't need separate function for each dose / column e.g. Dose1
```{r function-filter-non-prov-dose1}
# dose1 <- function(Date){
...
```{r funtion-filter-non-prov-date}
...
## Function Create a tooltip for % column values

```{r function-tooltip-column-percent}
tootip_per_col <- function(data, x, y){
    data %>%
      mutate(
...
```{r function-tootip-TEST-colnam}
tootip_per_col(bar_graph_data_recent_can, "TotalVaccinated")
```

## Function Create a tooltip for # column values

```{r function-tooltip-column-number}
...
### TEST function
need to fill in values tooltip_per_col(data, "columnname")  
creates a tooltext column in data objectm, with # symbol
```{r function_tootip_TEST}
tootip_num_col(bar_graph_data_recent_can, "TotalVaccinated")
```


### TEST Function tooltip for interactive bar chart 

```{r ggiraph-interactive-barchart-totvac-TEST}
...
## TEST Function Create a tooltip for comma-separated column values
- this gives comma format, but I can't seem to make a vunction to choose different columns to use  
- can at least use to choose which data to use, then manually change the column to use
```{r function-tooltip-column-comma-TEST}
...
4. make a function to avoid repeating code for different vaccine dose  
   - done:  >> tested both in a ggplot barchart
     - tootip
     - create data object with only Province (not not prov)
     - create data object, choosing only Date, and 2 columns !!
