...
# Data download & cleaning  >>> CanadaCensus-pop-age.Rmd
#-------- Function to create and dave a ggplot of Canadian census age and gender
census_age_gender_ggplot_save <- function(data, colpat1, colpat2, title, subtitle, caption, labelsize, filename, aspect_ratio = 2.5){
...
p1 <- census_age_gender_ggplot_save(data = Victoria_pop_age_gender_2021_seniors,
...
########### ---------------------------
#-------- A Function to create a ggplot for Canadian census age and gender
# Separate plot creation from filesave

census_age_gender_ggplot <- function(data, colpat1, colpat2, title, subtitle, caption, labelsize){
  ...
  # ggplot
  age_group_plot <- ggplot(data, aes(x = age_group, y = pop, fill = gender)) +
    geom_bar(stat = "identity", position = "dodge", alpha = 0.95, width = 0.85)  +
    # wideth = 1 no space between bars
    geom_text(aes(label = pop), size = labelsize, position = position_dodge(width = 0.9), vjust = 0.35, hjust = 1.05, angle = 90) +
    # vjust negative shows above bar
...
save_ggplot <- function(plot, filename, aspect_ratio = 2.5){
  aspect_ratio <- aspect_ratio
  ggsave(plot, filename, height = 10 , width = 4 * aspect_ratio)
}




p1_plot <- census_age_gender_ggplot(data = Victoria_pop_age_gender_2021_seniors,
                               colpat1 = "#af8dc3", colpat2 = "#7fbf7b",
                               title = "Victoria Seniors Population \n by Age & Gender",
                               subtitle = "(2021 Canada Census)",
                               caption = "Data: https://www12.statcan.gc.ca/census-recensement/2021",
...
p1_save <- save_ggplot(p1_plot, filename = "Victoria_pop_age_gender_2021_seniors_1.jpg"
)
p1_save
...
p2_plot <- census_age_gender_ggplot(data = BC_pop_2021,
                               colpat1 = "#af8dc3", colpat2 = "#7fbf7b",
                               title = "BC Population \n by Age & Gender",
                               subtitle = "(2021 Canada Census)",
                               caption = "Data: https://www12.statcan.gc.ca/census-recensement/2021",
...
p3_plot <- census_age_gender_ggplot(data = BC_pop_2021_seniors,
                               colpat1 = "#af8dc3", colpat2 = "#7fbf7b",
                               title = "BC Seniors Population \n by Age & Gender",
                               subtitle = "(2021 Canada Census)",
                               caption = "Data: https://www12.statcan.gc.ca/census-recensement/2021",
